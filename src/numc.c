...
static PyObject *Matrix61c_add(Matrix61c *self, PyObject *args)
{
    ...
    {
        PyErr_SetString(PyExc_TypeError, "Arguments must of type numc.Matrix!");
        return NULL;
    }
    ...
    {
        PyErr_SetString(PyExc_ValueError, "Matrices must be the same size!");
        return NULL;
    }

    // allocation memory check
    matrix *new_mat;
    int alloc_failed = allocate_matrix(&new_mat, self->mat->rows, self->mat->cols);
    if (alloc_failed != 0)
    {
        PyErr_SetString(PyExc_RuntimeError, "Failed to allocate resulting matrix");
        return NULL;
    }
...
static PyObject *Matrix61c_multiply(Matrix61c *self, PyObject *args)
{
    ...
    {
        PyErr_SetString(PyExc_ValueError, "Matrix A column must equal Matrix B rows");
        return NULL;
    }
...
static PyObject *Matrix61c_pow(Matrix61c *self, PyObject *pow, PyObject *optional)
{
    ...
    if (int_pow < 0)
    {
        PyErr_SetString(PyExc_ValueError, "Pow must be greater than or equal to 0");
        return NULL;
    }
...
static PyObject *Matrix61c_set_value(Matrix61c *self, PyObject *args)
{
    ...
    PyObject *row_index = NULL;
    PyObject *col_index = NULL;
    PyObject *input_value = NULL;
    if (!PyArg_UnpackTuple(args, "args", 3, 3, &row_index, &col_index, &input_value))
    {
        PyErr_SetString(PyExc_TypeError, "Must input 3 arguments!");
        return NULL;
    }

    if (!PyLong_Check(input_value) && !PyFloat_Check(input_value))
    {
        PyErr_SetString(PyExc_TypeError, "Arguments must of type int or float!");
        return NULL;
    }

    if (!PyLong_Check(row_index) || !PyLong_Check(col_index))
    {
        PyErr_SetString(PyExc_TypeError, "Row or column index must be of type int");
        return NULL;
    }

    int row = (int)PyLong_AsLong(row_index);
    int col = (int)PyLong_AsLong(col_index);

    if (row >= self->mat->rows || row < 0 || col >= self->mat->cols || col < 0)
    {
    ...
    if (PyFloat_Check(input_value))
    {
        val = (double)PyFloat_AsDouble(input_value);
    }
    else
    {
        val = (double)PyLong_AsDouble(input_value);
    }
...
static PyObject *Matrix61c_get_value(Matrix61c *self, PyObject *args)
{
    ...
    PyObject *row_index = NULL;
    PyObject *col_index = NULL;
    if (!PyArg_UnpackTuple(args, "args", 2, 2, &row_index, &col_index))
    {
        PyErr_SetString(PyExc_TypeError, "Must input 2 arguments!");
        return NULL;
    }
    ...
    double return_val = get(self->mat, row, col);
...
static PyMethodDef Matrix61c_methods[] = {
    /* TODO: YOUR CODE HERE */
    {"set", (PyCFunction)Matrix61c_set_value, METH_VARARGS, "Sets the index of a matrix to a specified value"},
    {"get", (PyCFunction)Matrix61c_get_value, METH_VARARGS, "Gets the value of an index within a matrix"},
    {NULL, NULL, 0, NULL}};
