from utils import *
from unittest import TestCase

"""
- For each operation, you should write tests to test  on matrices of different sizes.
- Keep in mind that the tests provided in the starter code are NOT comprehensive. That is, we strongly
advise you to modify them and add new tests.
- Hint: use dp_mc_matrix to generate dumbpy and numc matrices with the same data and use
      cmp_dp_nc_matrix to compare the results
"""


class TestAdd(TestCase):

    def test_typeError(self):
        # TODO: YOUR CODE HERE
        dp_mat1, nc_mat1 = rand_dp_nc_matrix(2, 2, seed=0)
        dp_mat2, nc_mat2 = rand_dp_nc_matrix(2, 2, seed=1)
        self.assertFalse(nc_mat1 + [1, 1, 1, 1])

    def test_dimensionError(self):
        # TODO: YOUR CODE HERE
        dp_mat1, nc_mat1 = rand_dp_nc_matrix(2, 2, seed=0)
        dp_mat2, nc_mat2 = rand_dp_nc_matrix(3, 3, seed=1)
        self.assertFalse(nc_mat1 + nc_mat2)

    def test_small_add(self):
        # TODO: YOUR CODE HERE
        dp_mat1, nc_mat1 = rand_dp_nc_matrix(2, 2, seed=0)
        dp_mat2, nc_mat2 = rand_dp_nc_matrix(2, 2, seed=1)
        is_correct, speed_up = compute(
            [dp_mat1, dp_mat2], [nc_mat1, nc_mat2], "add")
        dp_mat1 = dp_mat2 + dp_mat2
        nc_mat1 = nc_mat2 + nc_mat2

        self.assertEqual(dp_mat1, nc_mat1)

        self.assertTrue(is_correct)
        print_speedup(speed_up)

    def test_medium_add(self):
        # TODO: YOUR CODE HERE
        dp_mat1, nc_mat1 = rand_dp_nc_matrix(10, 10, seed=0)
        dp_mat2, nc_mat2 = rand_dp_nc_matrix(10, 10, seed=1)
        is_correct, speed_up = compute(
            [dp_mat1, dp_mat2], [nc_mat1, nc_mat2], "add")
        dp_mat1 = dp_mat2 + dp_mat2
        nc_mat1 = nc_mat2 + nc_mat2

        self.assertEqual(dp_mat1, nc_mat1)
        self.assertTrue(is_correct)
        print_speedup(speed_up)

    def test_large_add(self):
        # TODO: YOUR CODE HERE
        dp_mat1, nc_mat1 = rand_dp_nc_matrix(1500, 1500, seed=0)
        dp_mat2, nc_mat2 = rand_dp_nc_matrix(1500, 1500, seed=1)
        is_correct, speed_up = compute(
            [dp_mat1, dp_mat2], [nc_mat1, nc_mat2], "add")

        self.assertTrue(is_correct)
        print_speedup(speed_up)

        dp_mat1 = dp_mat2 + dp_mat2
        nc_mat1 = nc_mat2 + nc_mat2

        self.assertEqual(dp_mat1, nc_mat1)


class TestSub(TestCase):
    def test_typeError(self):
        # TODO: YOUR CODE HERE
        dp_mat1, nc_mat1 = rand_dp_nc_matrix(2, 2, seed=0)
        dp_mat2, nc_mat2 = rand_dp_nc_matrix(2, 2, seed=1)
        self.assertFalse(nc_mat1 - [1, 1, 1, 1])

    def test_dimensionError(self):
        # TODO: YOUR CODE HERE
        dp_mat1, nc_mat1 = rand_dp_nc_matrix(2, 2, seed=0)
        dp_mat2, nc_mat2 = rand_dp_nc_matrix(3, 3, seed=1)
        self.assertFalse(nc_mat1 - nc_mat2)

    def test_small_sub(self):
        # TODO: YOUR CODE HERE
        dp_mat1, nc_mat1 = rand_dp_nc_matrix(2, 2, seed=0)
        dp_mat2, nc_mat2 = rand_dp_nc_matrix(2, 2, seed=1)
        # is_correct, speed_up = compute(
        #     [dp_mat1, dp_mat2], [nc_mat1, nc_mat2], "add")
        dp_mat1 = dp_mat2 - dp_mat2
        nc_mat1 = nc_mat2 - nc_mat2

        self.assertEqual(dp_mat1, nc_mat1)

        # self.assertTrue(is_correct)
        # print_speedup(speed_up)

    def test_medium_sub(self):
        # TODO: YOUR CODE HERE
        dp_mat1, nc_mat1 = rand_dp_nc_matrix(10, 10, seed=0)
        dp_mat2, nc_mat2 = rand_dp_nc_matrix(10, 10, seed=1)
        # is_correct, speed_up = compute(
        #     [dp_mat1, dp_mat2], [nc_mat1, nc_mat2], "add")
        dp_mat1 = dp_mat2 - dp_mat2
        nc_mat1 = nc_mat2 - nc_mat2

        self.assertEqual(dp_mat1, nc_mat1)
        # self.assertTrue(is_correct)
        # print_speedup(speed_up)

    def test_large_sub(self):
        # TODO: YOUR CODE HERE
        dp_mat1, nc_mat1 = rand_dp_nc_matrix(100, 100, seed=0)
        dp_mat2, nc_mat2 = rand_dp_nc_matrix(100, 100, seed=1)
        # is_correct, speed_up = compute(
        #     [dp_mat1, dp_mat2], [nc_mat1, nc_mat2], "add")
        dp_mat1 = dp_mat2 - dp_mat2
        nc_mat1 = nc_mat2 - nc_mat2

        self.assertEqual(dp_mat1, nc_mat1)
        # self.assertTrue(is_correct)
        # print_speedup(speed_up)


class TestAbs(TestCase):

    def test_small_abs(self):
        # TODO: YOUR CODE HERE

        dp_mat, nc_mat = rand_dp_nc_matrix(3, 3, seed=0)

        nc_mat = abs(nc_mat)
        dp_mat = abs(dp_mat)
        self.assertEqual(nc_mat, dp_mat)

    def test_medium_abs(self):
        # TODO: YOUR CODE HERE
        dp_mat, nc_mat = rand_dp_nc_matrix(
            501, 221, low=-500, high=500, seed=0)
        is_correct, speed_up = compute([dp_mat], [nc_mat], "abs")

        self.assertTrue(is_correct)

    def test_large_abs(self):
        # TODO: YOUR CODE HERE

        dp_mat, nc_mat = rand_dp_nc_matrix(
            1501, 1221, low=-500, high=500, seed=0)
        dp_mat_2, nc_mat_2 = rand_dp_nc_matrix(
            1501, 1221, low=-500, high=500, seed=0)

        dp_mat_2 = abs(dp_mat)
        nc_mat_2 = abs(nc_mat)

        self.assertEqual(dp_mat_2, nc_mat_2)


class TestNeg(TestCase):
    def test_small_neg(self):
        # TODO: YOUR CODE HERE
        dp_mat, nc_mat = rand_dp_nc_matrix(2, 2, seed=0)
        is_correct, speed_up = compute([dp_mat], [nc_mat], "neg")
        self.assertTrue(is_correct)
        print_speedup(speed_up)

        dp_mat, nc_mat = rand_dp_nc_matrix(2, 2, low=-50, high=50, seed=0)

        nc_mat = -(nc_mat)
        dp_mat = -(dp_mat)
        self.assertEqual(nc_mat, dp_mat)

    def test_medium_neg(self):
        # TODO: YOUR CODE HERE
        dp_mat, nc_mat = rand_dp_nc_matrix(500, 500, seed=0)
        is_correct, speed_up = compute([dp_mat], [nc_mat], "neg")
        self.assertTrue(is_correct)
        print_speedup(speed_up)
        dp_mat, nc_mat = rand_dp_nc_matrix(50, 50, low=-50, high=50, seed=0)

        nc_mat = -(nc_mat)
        dp_mat = -(dp_mat)
        self.assertEqual(nc_mat, dp_mat)

    def test_large_neg(self):
        # TODO: YOUR CODE HERE
        dp_mat, nc_mat = rand_dp_nc_matrix(1500, 1500, seed=0)
        is_correct, speed_up = compute([dp_mat], [nc_mat], "neg")
        self.assertTrue(is_correct)
        print_speedup(speed_up)
        dp_mat, nc_mat = rand_dp_nc_matrix(
            500, 500, low=-500, high=500, seed=0)

        nc_mat = -(nc_mat)
        dp_mat = -(dp_mat)
        self.assertEqual(nc_mat, dp_mat)


class TestMul(TestCase):
    # def test_typeError(self):
    #     # TODO: YOUR CODE HERE
    #     dp_mat1, nc_mat1 = rand_dp_nc_matrix(2, 21, seed=0)
    #     dp_mat2, nc_mat2 = rand_dp_nc_matrix(20, 2, seed=1)
    #     self.assertFalse(nc_mat1 - [1, 1, 1, 1])

    # def test_dimensionError(self):
    #     # TODO: YOUR CODE HERE
    #     dp_mat1, nc_mat1 = rand_dp_nc_matrix(2, 21, seed=0)
    #     dp_mat2, nc_mat2 = rand_dp_nc_matrix(20, 2, seed=1)
    #     self.assertFalse(nc_mat1 * nc_mat2)

    # def test_small_mul(self):
    #     # TODO: YOUR CODE HERE
    #     dp_mat1, nc_mat1 = rand_dp_nc_matrix(2, 2, seed=0)
    #     dp_mat2, nc_mat2 = rand_dp_nc_matrix(2, 2, seed=1)
    #     # is_correct, speed_up = compute(
    #     #     [dp_mat1, dp_mat2], [nc_mat1, nc_mat2], "add")
    #     dp_mat1 = dp_mat2 * dp_mat2
    #     nc_mat1 = nc_mat2 * nc_mat2

    #     self.assertEqual(dp_mat1, nc_mat1)

    #     self.assertTrue(is_correct)
    #     print_speedup(speed_up)

    # def test_large_mul_medium(self):
    #     # TODO: YOUR CODE HERE
    #     dp_mat1, nc_mat1 = rand_dp_nc_matrix(12, 12, seed=0)
    #     dp_mat2, nc_mat2 = rand_dp_nc_matrix(12, 12, seed=0)
    #     is_correct, speed_up = compute(
    #         [dp_mat1, dp_mat2], [nc_mat1, nc_mat2], "mul")

    #     self.assertTrue(is_correct)
    #     print_speedup(speed_up)

    def test_large_mul_large(self):
        # TODO: YOUR CODE HERE
        dp_mat1, nc_mat1 = rand_dp_nc_matrix(1000, 1000, seed=0)
        dp_mat2, nc_mat2 = rand_dp_nc_matrix(1000, 1000, seed=0)
        is_correct, speed_up = compute(
            [dp_mat1, dp_mat2], [nc_mat1, nc_mat2], "mul")

        self.assertTrue(is_correct)
        print_speedup(speed_up)


class TestPow(TestCase):

    def test_pow_0(self):
        dp_mat, nc_mat = rand_dp_nc_matrix(
            10, 10, low=-50, high=50, seed=0)
        nc_mat = nc_mat**0
        dp_mat = dp_mat**0
        self.assertEqual(nc_mat, dp_mat)

    def test_non_square(self):
        dp_mat, nc_mat = rand_dp_nc_matrix(10, 9, low=-50, high=50, seed=0)

        self.assertFalse(nc_mat**2)

    def test_negative_pow(self):
        dp_mat, nc_mat = rand_dp_nc_matrix(10, 9, low=-50, high=50, seed=0)

        self.assertFalse(nc_mat**-2)

    def test_small_pow(self):
        # TODO: YOUR CODE HERE
        dp_mat, nc_mat = rand_dp_nc_matrix(2, 2, low=-50, high=50, seed=0)

        nc_mat = nc_mat**2
        dp_mat = dp_mat**2
        self.assertEqual(nc_mat, dp_mat)

        # is_correct, speed_up = compute([dp_mat, 3], [nc_mat, 3], "pow")
        # self.assertTrue(is_correct)
        # print_speedup(speed_up)

    def test_medium_pow(self):
        # TODO: YOUR CODE HERE
        dp_mat, nc_mat = rand_dp_nc_matrix(
            10, 10, low=-50, high=50, seed=0)

        nc_mat = nc_mat**4
        dp_mat = dp_mat**4
        self.assertEqual(nc_mat, dp_mat)

    def test_large_pow(self):
        # TODO: YOUR CODE HERE
        dp_mat, nc_mat = rand_dp_nc_matrix(
            300, 300, low=-50, high=50, seed=0)

        nc_mat = nc_mat**6
        dp_mat = dp_mat**6
        self.assertEqual(nc_mat, dp_mat)


class TestGet(TestCase):
    def test_get(self):
        # TODO: YOUR CODE HERE
        dp_mat, nc_mat = rand_dp_nc_matrix(2, 2, seed=0)
        rand_row = np.random.randint(dp_mat.shape[0])
        rand_col = np.random.randint(dp_mat.shape[1])
        self.assertEqual(round(dp_mat.get(rand_row, rand_col), decimal_places),
                         round(nc_mat.get(rand_row, rand_col), decimal_places))

    def test_get_large(self):
        # TODO: YOUR CODE HERE
        dp_mat, nc_mat = rand_dp_nc_matrix(1400, 1400, seed=0)
        rand_row = np.random.randint(dp_mat.shape[0])
        rand_col = np.random.randint(dp_mat.shape[1])
        self.assertEqual(round(dp_mat.get(rand_row, rand_col), decimal_places),
                         round(nc_mat.get(rand_row, rand_col), decimal_places))


class TestSet(TestCase):
    def test_set(self):
        # TODO: YOUR CODE HERE
        dp_mat, nc_mat = rand_dp_nc_matrix(1000, 1500, seed=0)
        rand_row = np.random.randint(dp_mat.shape[0])
        rand_col = np.random.randint(dp_mat.shape[1])

        dp_mat.set(rand_row, rand_col, 16.999)
        nc_mat.set(rand_row, rand_col, 16.999)
        self.assertTrue(cmp_dp_nc_matrix(dp_mat, nc_mat))
        self.assertEquals(nc_mat.get(rand_row, rand_col), 16.999)


class TestShape(TestCase):
    def test_shape(self):
        # TODO: YOUR CODE HERE
        dp_mat, nc_mat = rand_dp_nc_matrix(2, 2, seed=0)
        self.assertTrue(dp_mat.shape == nc_mat.shape)


class TestIndexGet(TestCase):
    def test_index_get(self):
        # TODO: YOUR CODE HERE
        dp_mat, nc_mat = rand_dp_nc_matrix(2, 2, seed=0)
        rand_row = np.random.randint(dp_mat.shape[0])
        rand_col = np.random.randint(dp_mat.shape[1])
        self.assertEqual(round(dp_mat[rand_row][rand_col], decimal_places),
                         round(nc_mat[rand_row][rand_col], decimal_places))


class TestIndexSet(TestCase):
    def test_set(self):
        # TODO: YOUR CODE HERE
        dp_mat, nc_mat = rand_dp_nc_matrix(2, 2, seed=0)
        rand_row = np.random.randint(dp_mat.shape[0])
        rand_col = np.random.randint(dp_mat.shape[1])
        dp_mat[rand_row][rand_col] = 2
        nc_mat[rand_row][rand_col] = 2
        self.assertTrue(cmp_dp_nc_matrix(dp_mat, nc_mat))
        self.assertEquals(nc_mat[rand_row][rand_col], 2)


class TestSlice(TestCase):
    def test_slice_basic(self):
        # TODO: YOUR CODE HERE
        dp_mat, nc_mat = rand_dp_nc_matrix(2, 2, seed=0)
        self.assertTrue(cmp_dp_nc_matrix(dp_mat[0], nc_mat[0]))
        self.assertTrue(cmp_dp_nc_matrix(dp_mat[1], nc_mat[1]))

    def test_slice_medium(self):
        # TODO: YOUR CODE HERE
        x, a = rand_dp_nc_matrix(10, 10, seed=0)
        b = a[0]
        del a
        c = b[0]
        del b

